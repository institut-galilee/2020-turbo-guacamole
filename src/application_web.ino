...
void loop() {
  ...
  if(cm > 0 && cm < 40){   // Si on détecte une présence et que la personne se connecte on allume les leds
    if (client) {
      Serial.println("Nouveau client");
      
      // On allume les led pour que la personne ait de la visibilité
          ...
          header += c;
           if (c == '\n') {   
            ...
            if (currentLine.length() == 0) {
              ...
              // Desactiver alarme
              if (header.indexOf("GET /alarme/desactiver") >= 0) {
                Serial.println("alarme desactiver");
                etat_alarme = "desactiver";
              ...
              client.println("<div  class=\"row justify-content-center no-gutters outer-div\">");
                ...
                // Desactivé alarme
                client.println("<div  class=\"col-xs-6 col-sm-6 col-md-6 col-lg-6 justify-content-center inner-div\">");
                  client.println("<p>Etat de l'alarme: " + etat_alarme + "</p>");      
                  if (etat_alarme=="desactiver") {
                    client.println("<p><a href=\"/alarme/desactiver\"><button class=\"button\">Vous avez desactiver l'alarme</button></a></p>");
                  } else {
                    client.println("<p><a href=\"/alarme/eteint\"><button type=\"button\" class=\"btn btn-info col-xs-6 col-sm-6 col-md-6 col-lg-6 button2\">Veuillez desactiver l'alarme</button></a></p>");
                  } 
                client.println("</div>");
      ...
      Serial.println("");
    } else { // detection d'une présence mais pas de connection on fait sonner l'alarme
        tone(buzzer, 440);
        delay(1000);
      }
  } else { // Pas de présence on allume pas les leds
    digitalWrite(led_presence1, LOW);
    digitalWrite(led_presence2, LOW);
    delay(100);
  }
